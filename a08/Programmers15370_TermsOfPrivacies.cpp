...
public:
    ...
    {
        int total_month = ((month & 1) || month == 7) ? 31 : 30;
        Date ret;
        ret.day = day + rhs.day;
        ret.month = month + rhs.month;
        if (ret.day > total_month) {
            ret.month += 1;
            ret.day -= total_month;
        }

        ret.year = year + rhs.year;
        if (ret.month > 12) {
            ret.year += 1;
            ret.month -= 12;
        }
...
vector<int> solution(string today, vector<string> terms, vector<string> privacies)
{
    ...
    unordered_map<char, Date> termsMap;
    for (const auto& t : terms) {
        stringstream ss{t};
        char type;
        int due;
        ss >> type >> due;
        termsMap.emplace(type, Date(due));
    }

    vector<pair<Date, Date>> d_privacies;
    d_privacies.reserve(privacies.size());

    vector<int> answer;

    for (int i = 0; i < privacies.size(); ++i) {
        Date d(privacies[i]);
        Date until = d + termsMap[privacies[i].back()];
        if (until <= today) {
