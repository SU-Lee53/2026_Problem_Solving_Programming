...
vector<int> solution(int n, vector<string> words) {
    ...
    for (int turn = 0; ;++turn) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            int currIdx = (turn * n) + (i);
            if (currIdx == words.size()) {
                answer.insert(answer.end(), {0,0});
                bEnd = true;
                break;
            }
            const string& curr = words[currIdx];
            if (words.begin() + currIdx != find(words.begin(), words.begin() + currIdx, curr)) {
                answer.insert(answer.end(), {i+1, turn+1});
                bEnd = true;
                break;
            }

            const string& before = words[currIdx - 1];
