// 작업
// http://210.93.60.51/problem/0191
// Description
//
...
vector<int> topology_sort(const vector<vector<int>>& g, vector<int>& indegree, int vertices, const vector<int>& time)
{
    ...
    while (!q.empty()) {
        ...
        for (int i : g[now]) {
            indegree[i] -= 1;
            dp[i] = max(dp[i], dp[now] + time[i]);  // now 작업 종료 후 i 작업 시간을 더한 것 중에서 최댓값
            if (indegree[i] == 0) {
                q.emplace(i);
            }
        }
...
int main()
{
    ...
    auto res = topology_sort(g, indegree, N, time_taken);
