...
// 2 초	256 MB	5677	3356	2638	59.428%
// 문제
...
int main()
{
    ...
    for (int i = 0; i < T; ++i) {
        ...
        std::cin >> nSize >> input >> target;

        int nWhiteMiss{}, nBlackMiss{};
        for (int i = 0; i < nSize; ++i) {
            if (input[i] != target[i]) {
                (input[i] == 'W') ? nWhiteMiss++ : nBlackMiss++;
            }
        }

        std::cout << std::max(nWhiteMiss, nBlackMiss) << std::endl;
    }
}