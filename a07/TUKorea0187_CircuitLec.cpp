...
// 각 대학에서 제시하는 d와 p값은 서로 다를 수도 있다. 이 학자는 이를 바탕으로,
// 가장 많은 돈을 벌 수 있도록 순회강연을 하려 한다. 강연의 특성상, 이 학자는 하루에 최대 한 곳에서만 강연을 할 수 있다.
//
...
int main()
{
    ...
    priority_queue<int, deque<int>, greater<>> pq;
    for (auto [p, d]: v) {
        pq.emplace(p);
        if (pq.size() > d) {
            pq.pop();
