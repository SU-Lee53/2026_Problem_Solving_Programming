...
// 1 초	256 MB	266087	82261	51397	27.624%
// 문제
...
bool check(const vector<int>& v, int mid, int targ)    // mid 주ㅇ간값으로 조건을 만족하는가
{
    long long total = 0;   // 잘린 나무 길이의 합
    for (int n : v) {
        if (n > mid) {
            total += (n - mid);
        }
...
int main()
{
    ...
    copy_n(istream_iterator<int>(cin), N, trees.begin());

    int start = 0;
    int end = 10'0000'0000;
    int answer = 0;
    while (start <= end) {
        int mid = (start + end) / 2;
