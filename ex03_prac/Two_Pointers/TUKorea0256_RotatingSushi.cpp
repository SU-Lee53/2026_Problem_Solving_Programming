// 회전 초밥
// http://210.93.60.51/problem/0256
// Description
//
...
//
// Input
// 첫 번째 줄에는 회전 초밥 벨트에 놓인 접시의 수 N, 초밥의 가짓수 d, 연속해서 먹는 접시의 수 k, 쿠폰 번호 c가
// 각각 하나의 빈 칸을 사이에 두고 주어진다.
//
...
int main()
{
    ...
    vector<int> dishes(N);
    copy_n(istream_iterator<int>{cin}, N, dishes.begin());

    vector<int> dish_counts(d+1, 0);
    int num_uniques = 0;
    // [0,k] 초기화
    for (int i = 0; i < k; ++i) {
        if (dish_counts[dishes[i]] == 0) {
            num_uniques++;
        }
        dish_counts[dishes[i]]++;
    }

    int result = num_uniques + (dish_counts[c] == 0 ? 1 : 0);


    for (int start = 1; start < N; ++start) {
        int end = (start + k - 1) % N;
        int add = dishes[end];
        int removed = dishes[start - 1];

        // 초밥 하나 제거
        --dish_counts[removed];
        if (dish_counts[removed] == 0) --num_uniques;   // 제거 후 갯수가 0이먄 unique 감소

        // 초밥 하나 추가
        // 그 전에 이미 있던건지 확인 필요
        if (dish_counts[add] == 0) ++num_uniques;
        ++dish_counts[add];

        // 쿠폰 초밥 확인하고 추가
        result = max(result, num_uniques + (dish_counts[c] == 0 ? 1 : 0));
    }
