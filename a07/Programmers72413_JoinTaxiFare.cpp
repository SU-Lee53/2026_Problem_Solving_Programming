...
vector<int> Dijikstra(int start, int n, const vector<vector<pair<int, int>>>& g)
{
...
vector<vector<int>> FloydWarshall(int n, const vector<vector<pair<int, int>>>& g)
{
    vector<vector<int>> distance(n+1, vector<int>(n+1, numeric_limits<int>::max()));
    for (int i = 1; i < n+1; ++i) {
        distance[i][i] = 0;
    }

    // 3중 루프 사용
    // i->j로 가는 비용보다 i->j->k 가 더 작으면 갱신
    for (int k = 1; k < n + 1; ++k) {
        for (int j = 1; j < n + 1; ++j) {
            for (int i = 1; i < n + 1; ++i) {
...
int solution(int n, int s, int a, int b, vector<vector<int>> fares)
{
    ...
    // auto dist_s = Dijikstra(s, n, graph);
    // auto dist_a = Dijikstra(a, n, graph);
    // auto dist_b = Dijikstra(b, n, graph);

    auto distance = FloydWarshall(n, graph);
