...
vector<int> solution(vector<string> id_list, vector<string> report, int k) {
    ...
    for (const auto& s : report) {
        auto delim = s.find(' ');
        auto reporter = s.substr(0, delim);
        auto reportee = s.substr(delim+1, s.size());
        auto isFirst = reportHistory[reporter].insert(reportee).second;
        if (isFirst) {
    ...
    for (int i = 0; i < res.size(); ++i) {
        auto it = reportHistory.find(id_list[i]);
        if (it == reportHistory.end()) continue;

        const auto& reportees = it->second;
        for (const auto& s : reportees) {
            if (reportCount[s] >= k) {
                ++res[i];
            }
        }
