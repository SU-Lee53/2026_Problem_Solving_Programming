...
int solution(int m, int n, vector<vector<int>> puddles)
{
    ...
    vector<vector<bool>> puddle_map(n, vector<bool>(m, false));
    for (const auto& p : puddles) {
        puddle_map[p[1]-1][p[0]-1] = true;
    }

    dp[0][0] = 1;   // 집부터 시작
    for (int r = 0; r < n; ++r) {
        for (int c = 0; c < m; ++c) {
            if ((r == 0 || c == 0) && !puddle_map[r][c]) {
                if (r == 0 && c > 0 && dp[r][c-1] != 0) dp[r][c] = 1;
                if (c == 0 && r > 0 && dp[r-1][c] != 0) dp[r][c] = 1;
            }
            else if (puddle_map[r][c]) dp[r][c] = 0;
