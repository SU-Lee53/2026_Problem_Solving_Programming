...
public:
    NumMatrix(vector<vector<int>>& matrix) {
        sum_table.resize(matrix.size() + 1, vector<int>(matrix[0].size() + 1, 0));
        for (int r = 1; r <= matrix.size(); ++r) {
            for (int c = 1; c <= matrix[0].size(); ++c) {
                sum_table[r][c] = sum_table[r][c-1] + sum_table[r-1][c] + matrix[r-1][c-1] - sum_table[r-1][c-1];
            }
        }
